...
start-airflow-cluster: build-externals
	...
	@ docker-compose -p ${ENVIRONMENT} --file ./infra/airflow-cluster/docker-compose.yaml --env-file ${ENV_FILE} up -d --force-recreate airflow-webserver-${ENVIRONMENT} airflow-scheduler-${ENVIRONMENT} airflow-triggerer-${ENVIRONMENT} airflow-flower-${ENVIRONMENT}

start-airflow-cluster-worker: build-externals
	@ echo -e "$(BUILD_PRINT)Starting Airflow services $(END_BUILD_PRINT)"
	@ docker-compose -p ${ENVIRONMENT} --file ./infra/airflow-cluster/docker-compose.yaml --env-file ${ENV_FILE} up -d --force-recreate airflow-worker-${ENVIRONMENT}

stop-airflow-cluster:
	@ echo -e "$(BUILD_PRINT)Stopping Airflow Cluster $(END_BUILD_PRINT)"
	@ docker-compose -p ${ENVIRONMENT} --file ./infra/airflow-cluster/docker-compose.yaml --env-file ${ENV_FILE} down airflow-webserver-${ENVIRONMENT} airflow-scheduler-${ENVIRONMENT} airflow-triggerer-${ENVIRONMENT} airflow-flower-${ENVIRONMENT}

stop-airflow-cluster-worker:
	@ echo -e "$(BUILD_PRINT)Stopping Airflow Cluster Worker $(END_BUILD_PRINT)"
	@ docker-compose -p ${ENVIRONMENT} --file ./infra/airflow-cluster/docker-compose.yaml --env-file ${ENV_FILE} down airflow-worker-${ENVIRONMENT}
