...
def smart_xcom_pull(key: str):
    ...
    selected_upstream_task_ids = [selected_task_id
                                  for selected_task_id in ti.xcom_pull(key=task_id, task_ids=ti.upstream_task_ids)
                                  if selected_task_id
                                  ]
    if selected_upstream_task_ids:
        return select_first_non_none(ti.xcom_pull(key=key, task_ids=selected_upstream_task_ids))
    return None
...
def smart_xcom_push(key: str, value: Any, destination_task_id: str = None):
    ...
    task_ids = [destination_task_id] if destination_task_id else ti.downstream_task_ids
    for task_id in task_ids:
        ti.xcom_push(key=task_id, value=current_task_id)
