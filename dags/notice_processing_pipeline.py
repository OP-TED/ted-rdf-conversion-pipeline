...
from dags.dags_utils import get_dag_param, smart_xcom_push, smart_xcom_forward, parse_notice_statuses_from_string, \
    smart_xcom_pull
...
def branch_selector(result_branch: str,
                    xcom_forward_keys: List[Any] = [NOTICE_IDS_KEY, NOTICES_WITH_STATUS_KEY]) -> str:
    ...
    notice_ids: List[str] = smart_xcom_pull(key=NOTICE_IDS_KEY)
    if notice_ids is None or len(notice_ids) < 1:
        result_branch = STOP_PROCESSING_TASK_ID
    for xcom_forward_key in xcom_forward_keys:
        smart_xcom_forward(key=xcom_forward_key, destination_task_id=result_branch)
    return result_branch


NOTICE_SUCCESS_STATUSES: List[NoticeStatus] = parse_notice_statuses_from_string(config.NOTICE_SUCCESS_STATUSES)
