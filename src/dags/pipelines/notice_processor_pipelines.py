...
def notice_transformation_pipeline(notice: Notice, mongodb_client: MongoClient) -> NoticePipelineOutput:
    ...
    try:
        ...
        if not result:
            log_notice_error(
                message=f"This notice {notice.ted_id} is not eligible for transformation. Notice info: "
                        ...
                        f"eform_sdk_version=[{notice.normalised_metadata.eform_sdk_version}]. Check mapping packages!",
        ...
        log_notice_error(
            ...
            domain_action=notice_transformation_pipeline.__name__,
            notice_status=str(notice.status),
            notice_form_number=notice.normalised_metadata.form_number,
            notice_eforms_subtype=notice.normalised_metadata.eforms_subtype)
        return NoticePipelineOutput(notice=notice, processed=False, store_result=True)
...
def notice_validation_pipeline(notice: Notice, mongodb_client: MongoClient) -> NoticePipelineOutput:
    ...
    try:
        ...
        log_notice_error(
            message=str(e),
            notice_id=notice.ted_id,
            domain_action=notice_validation_pipeline.__name__,
            notice_status=str(notice.status),
            notice_form_number=notice.normalised_metadata.form_number,
            notice_eforms_subtype=notice.normalised_metadata.eforms_subtype)
        return NoticePipelineOutput(notice=notice, processed=False, store_result=True)
...
def notice_package_pipeline(notice: Notice, mongodb_client: MongoClient = None) -> NoticePipelineOutput:
    ...
    """
    from src.ted_sws.notice_packager.services.notice_packager import package_notice
    ...
    try:
        ...
        log_notice_error(
            message=str(e),
            notice_id=notice.ted_id,
            domain_action=notice_package_pipeline.__name__,
            notice_status=str(notice.status),
            notice_form_number=notice.normalised_metadata.form_number,
            notice_eforms_subtype=notice.normalised_metadata.eforms_subtype)
        return NoticePipelineOutput(notice=notice, processed=False, store_result=True)
...
def notice_publish_pipeline(notice: Notice,
    ...
    """
    from src.ted_sws.notice_publisher.services.notice_publisher import publish_notice, publish_notice_rdf_into_s3, \
