...
def extract_text_from_elements(elements: List[Element]) -> List[str]:
    return [element.text for element in elements]


def extract_attribute_value_from_elements(elements: List[Element], attrib_key: str) -> List[str]:
...
class XMLManifestationMetadataExtractor:
    ...
    @property
    def title(self):
        title_text = extract_text_from_elements(self.manifestation_root.findall(
        ...
        return extract_text_from_elements(self.manifestation_root.findall(
    ...
    def name_of_buyer(self):
        ...
        if len(elements_found) == 1:
            return extract_text_from_elements(elements=elements_found)
        else:
            return extract_text_from_elements(
    ...
    def country_of_buyer(self):
        return extract_attribute_value_from_elements(self.manifestation_root.findall(
    ...
    def place_of_performance(self):
        return extract_attribute_value_from_elements(elements=self.manifestation_root.findall(
            "manifestation_ns:CODED_DATA_SECTION/manifestation_ns:NOTICE_DATA/manifestation_ns:PERFORMANCE_NUTS",
            namespaces=self.namespaces), attrib_key="CODE") or extract_attribute_value_from_elements(
    ...
    def legal_basis_directive(self):
        return extract_attribute_value_from_elements(elements=self.manifestation_root.findall(
            ...
            namespaces=self.namespaces), attrib_key="VALUE") or extract_attribute_value_from_elements(
            elements=self.manifestation_root.findall(
                "manifestation_ns:FORM_SECTION/*/manifestation_ns:LEGAL_BASIS",
                namespaces=self.namespaces), attrib_key="VALUE") or extract_text_from_elements(
            elements=self.manifestation_root.findall(
