...
CONCEPTUAL_MAPPINGS_FILE_TEMPLATE = '{mappings_path}/{mapping_package_id}/' + MS_TRANSFORM_FOLDER_NAME + '/' \
                                    + MS_CONCEPTUAL_MAPPING_FILE_NAME
MAPPING_PACKAGE_FILE_TEMPLATE = '{mappings_path}/{mapping_package_id}'


def test_mapping_package_processor_inject_resources(fake_mapping_package_id, file_system_repository_path,
                                                  resources_files_path):
    with tempfile.TemporaryDirectory() as temp_folder:
        temp_mapping_package_path = Path(temp_folder)
        shutil.copytree(file_system_repository_path, temp_mapping_package_path, dirs_exist_ok=True)

        conceptual_mappings_file_path = Path(CONCEPTUAL_MAPPINGS_FILE_TEMPLATE.format(
            mappings_path=temp_mapping_package_path,
            mapping_package_id=fake_mapping_package_id
        ))

        output_folder_path = temp_mapping_package_path / "_mappings_files"
...
def test_mapping_package_processor_inject_rml_modules(fake_mapping_package_id, file_system_repository_path,
                                                    rml_modules_path):
    with tempfile.TemporaryDirectory() as temp_folder:
        temp_mapping_package_path = Path(temp_folder)
        shutil.copytree(file_system_repository_path, temp_mapping_package_path, dirs_exist_ok=True)
        ...
        output_folder_path = temp_mapping_package_path / "_rml_modules"
        output_folder_path.mkdir(exist_ok=True)
...
def test_mapping_package_processor_inject_shacl_shapes(fake_mapping_package_id, file_system_repository_path,
                                                     resources_shacl_files_path):
    with tempfile.TemporaryDirectory() as temp_folder:
        temp_mapping_package_path = Path(temp_folder)
        shutil.copytree(file_system_repository_path, temp_mapping_package_path, dirs_exist_ok=True)

        output_folder_path = temp_mapping_package_path / "_shacl_shapes"
        output_folder_path.mkdir(exist_ok=True)
...
def test_mapping_package_processor_inject_shacl_shape(fake_mapping_package_id, file_system_repository_path,
                                                    resources_shacl_files_path):
    with tempfile.TemporaryDirectory() as temp_folder:
        temp_mapping_package_path = Path(temp_folder)
        shutil.copytree(file_system_repository_path, temp_mapping_package_path, dirs_exist_ok=True)
...
def test_mapping_package_processor_inject_sparql_queries(fake_mapping_package_id, file_system_repository_path,
                                                       resources_sparql_files_path):
    with tempfile.TemporaryDirectory() as temp_folder:
        temp_mapping_package_path = Path(temp_folder)
        shutil.copytree(file_system_repository_path, temp_mapping_package_path, dirs_exist_ok=True)

        output_folder_path = temp_mapping_package_path / "_sparql_queries"
...
def test_mapping_package_processor_inject_integration_sparql_queries(fake_mapping_package_id, file_system_repository_path,
                                                                   resources_sparql_files_path):
    with tempfile.TemporaryDirectory() as temp_folder:
        temp_mapping_package_path = Path(temp_folder)
        shutil.copytree(file_system_repository_path, temp_mapping_package_path, dirs_exist_ok=True)

        output_folder_path = temp_mapping_package_path / "_integration_sparql_queries"
        output_folder_path.mkdir(exist_ok=True)
