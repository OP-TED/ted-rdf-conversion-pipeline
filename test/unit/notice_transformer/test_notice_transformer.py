...
def test_notice_transformer_function(fake_rml_mapper, fake_mapping_package, notice_2018):
    notice_2018._status = NoticeStatus.PREPROCESSED_FOR_TRANSFORMATION
    result_notice = transform_notice(notice_2018, fake_mapping_package, fake_rml_mapper)
    assert result_notice.status == NoticeStatus.TRANSFORMED


def test_notice_transformer_by_id_function(fake_rml_mapper, mongodb_client, fake_mapping_package, notice_2018,
                                           notice_repository):
    notice_2018._status = NoticeStatus.PREPROCESSED_FOR_TRANSFORMATION
...
def test_notice_transformer_by_id_function_with_invalid_ids(fake_rml_mapper, mongodb_client, fake_mapping_package,
                                                            notice_2018,
    ...
    with pytest.raises(Exception):
        transform_notice_by_id(notice_id, fake_mapping_package.identifier, notice_repository, mapping_package_repository,
                               fake_rml_mapper)
    result_notice = notice_repository.get(reference=notice_id)
...
def test_transform_test_data_function(fake_rml_mapper, fake_mapping_package):
    with tempfile.TemporaryDirectory() as d:
        output_path = pathlib.Path(d)
        transform_test_data(mapping_package=fake_mapping_package, rml_mapper=fake_rml_mapper, output_path=output_path)
        file_names = [path.stem for path in output_path.glob("**/*.ttl")]
        test_data = fake_mapping_package.transformation_test_data.test_data
...
def test_transform_test_data_function_with_notices(fake_rml_mapper, fake_mapping_package):
    with tempfile.TemporaryDirectory() as d:
        output_path = pathlib.Path(d)
